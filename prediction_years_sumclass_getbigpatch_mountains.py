...
def get_raster_files_by_year(folder_path, tile_name, year, orbit_directions):
    ...
    for orbit_direction_item in orbit_directions:
        ...
        pattern = os.path.join(folder_path, orbit_direction_item, f'{tile_name}_*_{year}*CLASS.tif')
...
if __name__ == "__main__":
    ...
    for tile_item in tiles:
        folder_path = f'/mnt/hddarchive.nfs/amazonas_dir/output/ai_detection/{model}/{tile_item}'
        ...
        # for year in years:
        #
        #     output_path = Path(f'/mnt/hddarchive.nfs/amazonas_dir/output/ai_detection/{model}/{tile_item}/predictionyears_classsummask_{tile_item}_{year}.tif')
        #     if not output_path.exists():
        #         main(folder_path, tile_item, year, output_path, orbit_directions)

        output_path = Path(f'/mnt/hddarchive.nfs/amazonas_dir/output/ai_detection/{model}/{tile_item}/predictionyears_classsummask_{tile_item}_all.tif')
        if not output_path.exists():
            sum_all(folder_path, tile_item, years, str(output_path), orbit_directions)

        output_sieved_path = Path(
            f'/mnt/hddarchive.nfs/amazonas_dir/output/ai_detection/{model}/{tile_item}/predictionyears_classsummask_{tile_item}_all_sieved.tif')
        if output_sieved_path.exists():
            output_sieved_path.unlink(missing_ok=True)
            create_sieved(output_path, output_sieved_path, sieve=100000)
